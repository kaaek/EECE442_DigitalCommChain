...
if isfield(opts,'A_design') && ~isempty(opts.A_design)
...
% avg length 
sym2p = containers.Map('KeyType','char','ValueType','double');
for k = 1:numel(A_observed)
    sym2p(char(A_observed(k))) = double(p(k));
end
...
codeMap = containers.Map('KeyType','char','ValueType','char');
for k = 1:numel(keys)
    codeMap(keys{k}) = values{k};
end
...
R.tree_root = root;
if isfield(opts,'notes'); R.notes = opts.notes; end
...
nodes = struct('sym',{},'p',{},'left',{},'right',{}); edges = [];
    function id = addNode(n)
        id = numel(nodes) + 1;
        nodes(id).sym   = n.sym; nodes(id).p = n.p;
        nodes(id).left  = []; nodes(id).right = [];
        if ~isempty(n.left)
            L = addNode(n.left); nodes(id).left = L; edges(end+1,:) = [id, L, 0]; 
        end
        if ~isempty(n.right)
            R = addNode(n.right); nodes(id).right = R; edges(end+1,:) = [id, R, 1]; 
        end
    end
addNode(root);

n = numel(nodes); pos = zeros(n,2); leafX = 0;
    function assignPos(id, depth)
        isLeaf = isempty(nodes(id).left) && isempty(nodes(id).right);
        if isLeaf
            leafX = leafX + 1; pos(id,:) = [leafX, depth];
        else
...
if darkMode
    edgeC = [1 1 1]; txtC = [1 1 1]; faceC = [0.2 0.2 0.2]; edgeNode = [1 1 1];
else
    edgeC = [0.5 0.5 0.5]; txtC = [0 0 0]; faceC = [0.92 0.92 0.92]; edgeNode = [0 0 0];
end
